...
- 同一次点击（同一元素/同一选择器）最多尝试 2 次，禁止反复换选择器死磕同一个元素；
...
页面动作前，若当前页面存在遮挡层，必须先处理掉再继续，否则点击会一直失败并白耗时间：

1. **识别遮挡**：广告屏蔽提示、插件提示、登录引导、新手引导、活动浮层、Cookie/隐私提示、全屏遮罩（例如“我知道了”“关闭”“×”“继续浏览”“以后再说”类按钮），以及盖在卡片标题上的封面遮罩（`a.cover`、`mask` 一类）。
2. **只点关闭类按钮**：用 fresh snapshot 找到关闭/跳过按钮并点击一次，确认遮罩消失。禁止点击遮罩里的下载、安装、去登录、领取、购买、立即体验等动作；禁止向任何弹窗或输入框输入内容。
3. **推荐流卡片被遮罩盖住标题时**：不要死磕标题链接。改用同一张卡片在 snapshot 中的封面链接或卡片容器 ref 点击（封面点击同样会进入帖子）。优先使用 snapshot 里的 ref，不要自己拼 CSS 选择器或猜坐标。
4. **失败就换，不要循环**：同一个元素/选择器连续失败 2 次，立即改为别的动作继续推进——优先换相邻卡片、或按预算向下滚动一次、或按预算重新打开首页一次。禁止连续尝试第三种、第四种选择器，也禁止用 `evaluate`、DOM 查询或坐标点击绕开。
5. **遮罩关不掉时**：只要能正常读取到内容就继续；完全无法读取且已用完上面允许的动作时，输出 `BROWSER_ERROR`，不要无限重试。
...
4. 点击页面顺序中的第一篇相关帖子，不比较热度，不寻找“更优帖子”。点击前先按「遮罩与弹窗处理」清掉遮挡层；标题链接被封面遮罩盖住时，改点同一张卡片的封面链接或卡片容器 ref，不要反复换选择器；同一元素连续失败 2 次就换相邻卡片。每次打开帖子都将 `posts_checked` 加一；最多打开 2 篇。
