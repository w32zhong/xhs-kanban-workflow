...
## v0.53 — scout 补上遮罩与弹窗处理规则

- 现象：某轮 scout 第 1 次尝试跑满 8.5 分钟被强制超时（`elapsed 508s > limit 480s`）。日志显示它连续点击推荐流卡片失败（`click e41`、`click e40` 均返回 exit 1），worker 自身判断"标题链接被 `a.cover.mask` / `div.footer` 遮罩盖住"，随后不断更换选择器（含自拼 CSS 选择器）死磕同一个元素，直到撞上时间上限；重试的第二次尝试虽然正常通过 S1，但整轮 scout 耗时接近正常的 3 倍。
- 根因：v0.52 只给「精确发布」补了遮挡弹窗规则，**搜索阶段的 scout prompt 没有同类规则**，遇到封面遮罩挡住标题链接时没有任何退出路径。
- 修订（仅改 `prompts/scout-worker.md`，不动任何预算）：
  - 新增「遮罩与弹窗处理（强制）」小节：识别广告屏蔽/插件/登录引导/新手引导/活动浮层/Cookie 提示/全屏遮罩以及盖住标题的封面遮罩；只点关闭类按钮并确认遮罩消失；禁止点击遮罩内的下载、安装、去登录、领取、购买等动作，禁止向弹窗输入内容；遮罩关不掉但内容可读则继续，完全不可读且允许动作已用完才 `BROWSER_ERROR`。
  - 明确卡片点击策略：标题被遮罩盖住时改点同一张卡片的封面链接或卡片容器 ref，优先用 snapshot 的 ref，**不要自拼 CSS 选择器或猜坐标**；同一元素连续失败 2 次就换相邻卡片、或按预算滚动一次、或按预算重开首页一次；禁止用 `evaluate`、DOM 查询或坐标点击绕开。
  - 硬预算新增一条：同一次点击（同一元素/同一选择器）最多尝试 2 次，禁止反复换选择器死磕同一元素。
  - S3 第 4 步改为：点击前先清遮挡层，标题被遮罩时改点封面链接或卡片容器 ref，同一元素连续失败 2 次就换相邻卡片；`posts_checked` 与"最多打开 2 篇"的约束不变。
- 预算未调整：scout 仍为 `max_runtime=8m`，2A/2B 仍为 4m，委员长仍为 4m，发布仍为 8m，复核仍为 6m。
- 未改动：`run.py`、其他 prompt、schema、发布与复核路径。未新增测试文件。
